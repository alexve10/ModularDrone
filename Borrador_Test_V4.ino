...
///////////////////////////// PID ///////////////////////////
int JS1_Y_throttle, JS2_Y_pitch, JS2_X_roll, JS1_X_yaw, throttle;
;
...
float tiempo_prev, dt = 1;
float kp_roll = 0.1, ki_roll = 0.005, kd_roll = 0.2;
...
void loop() {
  ...
  if (error == 1)  //skip loop if no controller found
    ...
    if (state == 1) {
      ...
      JS1_Y_throttle = ps2x.Analog(PSS_LY);
      JS2_Y_pitch = ps2x.Analog(PSS_RY);
      JS2_X_roll = ps2x.Analog(PSS_RX);
      JS1_X_yaw = ps2x.Analog(PSS_LX);

      JS1_Y_throttle = constrain(JS1_Y_throttle, 0, 127);
      throttle = map(JS1_Y_throttle, 127, 0, 0, 20);
      ang_pitch_des = map(JS2_Y_pitch, 0, 254, -30, 30);
      ang_roll_des = map(JS2_X_roll, 0, 254, -30, 30);
...
void ISR_Timer() {
  ...
  i_pitch = constrain(i_pitch, -30, 30);
  d_pitch = kd_pitch * ((err_pitch - err_pitch_prev) / SampleTime);
